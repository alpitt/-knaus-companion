...
test("service worker offline shell is version-consistent and complete",()=>{
  ...
  const core=vm.runInNewContext(match[1]);assert.ok(core.includes("./index.html"));assert.ok(core.includes("./manifest.webmanifest"));assert.match(worker,/v18-0-0-ownership-intelligence/);
...
test("release version, storage and fallback remain consistent",()=>{
  ...
  assert.equal(html,fallback);assert.match(html,/Knaus Companion 18\.0\.0/);assert.match(source,/const APP_VERSION="18\.0\.0"/);assert.equal(build.version,"18.0.0");assert.match(source,/const STORE_KEY="knaus-ultimate-v1"/);assert.match(source,/DEFAULT_STATE=\{schemaVersion:2/);
});
