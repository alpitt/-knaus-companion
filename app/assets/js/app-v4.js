...
function renderSeasonalCustomTasks(){const tasks=(state.seasonalCustomTasks||[]).filter(item=>item.mode===activeSeasonalMode);$("#seasonalCustomTasks").innerHTML=tasks.length?tasks.map(item=>`<article class="panel seasonal-item ${item.complete?"complete":""}"><button class="seasonal-check" data-seasonal-custom-toggle="${item.id}" role="checkbox" aria-checked="${Boolean(item.complete)}"><span>${item.complete?"✓":""}</span><strong>${esc(item.title)}</strong></button><div><button class="secondary-btn" data-route="${item.route}">Guidance</button><button class="danger-btn" data-seasonal-custom-delete="${item.id}">Delete</button></div></article>`).join(""):'<article class="panel"><p>No custom tasks for this seasonal mode.</p></article>'}
function saveSeasonalCustomTask(event){event.preventDefault();const values=Object.fromEntries(new FormData(event.currentTarget)),item={id:`seasonal-custom-${Date.now()}`,mode:activeSeasonalMode,title:values.title.trim(),route:values.route,complete:false,createdAt:new Date().toISOString()};state.seasonalCustomTasks=[item,...(state.seasonalCustomTasks||[])];saveState();event.currentTarget.reset();renderSeasonal();toast("Custom seasonal task added")}
...
function deleteSeasonalCustomTask(id){const item=(state.seasonalCustomTasks||[]).find(entry=>entry.id===id);if(!item||!confirm(`Delete custom task “${item.title}”?`))return;state.seasonalCustomTasks=state.seasonalCustomTasks.filter(entry=>entry.id!==id);saveState();renderSeasonal();toast("Custom seasonal task deleted")}
...
function seasonalCalendarEvents(){return SEASONAL_CHECKLISTS.map(list=>{const plan=state.seasonalPlans?.[list.id];if(!plan?.targetDate)return null;const done=list.items.filter(([id])=>state.seasonalProgress?.[`${list.id}:${id}`]).length;return{id:list.id,date:plan.targetDate,title:list.title,detail:`${done}/${list.items.length} checks complete${plan.notes?` • ${plan.notes}`:""}`,complete:done===list.items.length}}).filter(Boolean).sort((a,b)=>a.date.localeCompare(b.date))}
function renderSeasonalCalendar(){const events=seasonalCalendarEvents(),today=new Date().toISOString().slice(0,10);$("#seasonalCalendar").innerHTML=events.length?events.map(item=>`<button class="panel seasonal-calendar-event ${item.date<today&&!item.complete?"overdue":""} ${item.complete?"complete":""}" data-seasonal-mode="${item.id}"><span>${item.complete?"✓":"❄"}</span><span><strong>${esc(item.title)}</strong><small>${esc(formatTripDate(item.date))} • ${esc(item.detail)}</small></span><b>→</b></button>`).join(""):'<article class="panel"><p>Add a target date to a seasonal plan to build this calendar.</p></article>'}
