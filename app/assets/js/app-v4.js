...
function renderEmergency(){
  ...
  $("#emergencySummary").innerHTML=[[contacts.length,"Saved contacts"],[openIncidents,"Open incidents"],[openFaults,"Open faults"],[state.currentMileage||0,"Current km"]].map(([value,label])=>`<article class="stat-card"><strong>${esc(value)}</strong><span>${label}</span></article>`).join("");
...
function saveEmergencyIncident(event){event.preventDefault();const values=Object.fromEntries(new FormData(event.currentTarget)),existing=(state.emergencyIncidents||[]).find(item=>item.id===editingEmergencyIncidentId),item={id:existing?.id||`incident-${Date.now()}`,title:values.title.trim(),occurredAt:values.occurredAt,location:values.location.trim(),category:values.category,severity:values.severity,status:values.status,contacted:values.contacted.trim(),actions:values.actions.trim(),followUp:values.followUp.trim(),createdAt:existing?.createdAt||new Date().toISOString(),updatedAt:new Date().toISOString()};state.emergencyIncidents=existing?state.emergencyIncidents.map(entry=>entry.id===existing.id?item:entry):[item,...(state.emergencyIncidents||[])];saveState();closeEmergencyIncidentEditor();renderEmergency();renderHome();toast(existing?"Incident updated":"Incident recorded")}
