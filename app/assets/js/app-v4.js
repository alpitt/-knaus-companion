...
function renderHeatingRuns(){const runs=[...(state.heatingRuns||[])].sort((a,b)=>String(b.date).localeCompare(String(a.date))||String(b.createdAt).localeCompare(String(a.createdAt))),action=runs.filter(item=>item.result==="action").length;if(!$("#heatingRunDate").value)$("#heatingRunDate").value=new Date().toISOString().slice(0,10);$("#heatingRunSummary").textContent=runs.length?`${runs.length} tests • ${action} action required`:"No tests recorded";$("#heatingRunHistory").innerHTML=runs.length?runs.map(item=>`<article class="panel heating-run-card result-${item.result}"><span>${item.result==="normal"?"✓":item.result==="monitor"?"!":"⚠"}</span><div><small>${esc(formatTripDate(item.date))} • ${esc(item.mode)} • ${Number(item.duration)} min</small><strong>${item.result==="action"?"Action required":item.result==="monitor"?"Monitor":"Normal operation"}</strong>${item.faultCode?`<p><b>Fault:</b> ${esc(item.faultCode)}</p>`:""}${item.notes?`<p>${esc(item.notes)}</p>`:""}</div><button class="danger-btn" data-heating-run-delete="${item.id}">Delete</button></article>`).join(""):'<article class="panel"><p>No heating test runs recorded yet.</p></article>'}
