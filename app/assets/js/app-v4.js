...
function applicationRecordCounts(){return{service:(state.logs||[]).length,faults:(state.faults||[]).length,measurements:(state.electricalMeasurements||[]).length,trips:(state.trips||[]).length,documents:(state.vehicleDocuments||[]).length,photos:Object.keys(state.vehiclePhotoNotes||{}).length,assistant:(state.assistantHistory||[]).length,ownerRecords:(state.ownerRecords||[]).length,evidenceLinks:(state.evidenceLinks||[]).length,revisions:(state.ownerRecords||[]).reduce((sum,record)=>sum+(record.history?.length||0),0)}}
...
function reasoningRuntimeRecords(){const make=(id,sourceType,title,value,route)=>({id,sourceType,sourceId:id,title,summary:`${value.archived?"Historical archived":"Current owner"} record: ${title}. Status: ${value.status||"recorded"}${value.occurredAt?` at ${value.occurredAt}`:""}.`,text:JSON.stringify(value),keywords:value.tags||[],systems:value.systemId?[value.systemId]:[],componentIds:value.componentIds||[],manualPages:value.manualPageRefs||[],chapterIds:value.chapterRefs||[],kbIds:value.kbRefs||[],evidenceRefs:value.evidenceRefs||[],confidence:value.confidence||"high",safetyClass:value.safetyClass||value.metadata?.safetyClass||"informational",route,sectionAnchor:"",sourceFile:"saved-state",archived:Boolean(value.archived),occurredAt:value.occurredAt||value.createdAt,searchableText:`${title} ${JSON.stringify(value)}`.toLowerCase(),checksum:"runtime"});return[...(state.faults||[]).map((x,i)=>make(`runtime:fault:${x.id||i}`,"fault-record",x.title||"Fault record",x,"#diagnostics")),...(state.electricalMeasurements||[]).map((x,i)=>make(`runtime:measurement:${x.id||i}`,"measurement",`${x.testPoint||"Electrical"} measurement`,x,"#electrical")),...(state.logs||[]).map((x,i)=>make(`runtime:maintenance:${x.id||i}`,"owner-record",x.title||"Service record",x,"#maintenance")),...(state.ownerRecords||[]).map((x,i)=>make(`runtime:owner:${x.id||i}`,`owner-${x.type}`,x.title||"Owner record",x,"#records"))]}
...
function parseBackupText(text){
  ...
  const incoming=payload?.state||payload;if(!incoming||typeof incoming!=="object"||Array.isArray(incoming))throw new Error("Backup structure is invalid. No data was changed.");if(incoming.ownerRecords!==undefined&&!Array.isArray(incoming.ownerRecords))throw new Error("Backup owner records are invalid. No data was changed.");if(Array.isArray(incoming.ownerRecords)&&globalThis.KnausOwnerRecords){const ids=incoming.ownerRecords.map(record=>record.id),invalid=incoming.ownerRecords.filter(record=>!globalThis.KnausOwnerRecords.validate(record,{knownRecordIds:ids}).valid);if(invalid.length)throw new Error(`Backup contains ${invalid.length} invalid owner record${invalid.length===1?"":"s"}. No data was changed.`)}
...
async function init(){
  ...
  const ownerAdapter={getState:()=>state,setState:next=>{state=next;saveState()}};window.KnausOwnerRecords?.initialise(ownerAdapter);window.KnausEvidenceLinks?.initialise(ownerAdapter);
