...
function renderResults(target,items,empty="No matching results."){
  ...
  root.innerHTML=items.map((x,i)=>`<article class="result-card" data-result="${i}" tabindex="0" role="button"><span class="meta">${esc(x.type)}</span><h3>${esc(x.title)}</h3>${x.evidence?`${evidenceBadge(x.evidence)}<p>${esc(target==="#assistantResults"?vehicleEvidenceAnswer(x):String(x.evidence.value||"Unknown"))}</p>${x.evidence.source?`<small>Source: ${esc(x.evidence.source)}</small>`:""}`:`<p>${esc(String(x.text||"").slice(0,280))}</p>`}<span class="open-hint">Tap to open</span></article>`).join("");
