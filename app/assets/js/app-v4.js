...
const DATA={chapters:[],pages:[],diagnostics:[],maintenanceTasks:[],assistantPrompts:[],build:null,electrical:[],water:[],gas:[],campsites:[],touringChecks:[]};
let state=loadState();
...
const NAV=[
  ...
  ["maintenance","Service & maintenance","⚙"],["diagnostics","Diagnostics","✓"],["touring","Touring","➜"],["vehicle","My motorhome","▣"],["settings","Settings","⋯"]
];
function renderNav(){
...
async function init(){
  [
    ...
    DATA.electrical,DATA.water,DATA.gas,DATA.campsites,DATA.touringChecks
  ]=await Promise.all([
    ...
    loadJSON("data/electrical.json"),loadJSON("data/water.json"),loadJSON("data/gas.json"),
    loadJSON("data/campsites.json"),loadJSON("data/touring_checklists.json")
  ]);
  applyTheme();renderNav();renderHome();renderAssistant();renderLibrary();renderMaintenance();renderDiagnostics();renderTouring();renderVehicle();renderSettings();
