...
Version 14.1.0 adds the read-only Digital Twin foundation: repository-supported vehicle identity, systems, components, relationships, evidence, legacy-state mapping, data-quality reporting and offline search. Unknown details remain explicit.
...
Version 10.6.0 adds Custom Seasonal Tasks. Add vehicle-specific work to storage, winterisation or reactivation, associate each task with relevant system guidance, track completion independently and remove tasks that no longer apply.
...
Version 9.6.0 adds Emergency Drills. Record isolation, evacuation, roadside, fire, medical and security practice with participants, duration, outcome, lessons and a next review date; overdue reviews appear on the Operations Dashboard and recent drills flow into the Emergency Handoff.

Version 9.5.0 adds the Emergency Handoff. Create one responder and recovery pack containing vehicle identity, the full readiness checklist, assistance contacts, unresolved incidents and emergency notes, then print it cleanly or export it as structured JSON.
...
Version 9.1.0 adds the Emergency Incident Log. Record breakdowns, collisions, safety concerns and recovery events with date, location, category, severity, actions, assistance and follow-up; track open, monitoring and resolved states; and surface unresolved incidents on the Operations Dashboard.

Version 9.0.0 begins Emergency Preparedness with a dedicated offline Emergency Centre. Keep vehicle identity and mileage ready, open gas, electrical, water and fault guidance, manage owner-controlled insurer, recovery, service and personal contacts with tap-to-call actions, and retain emergency notes in backups.

Version 8.4.0 adds the Compliance Calendar. Review all overdue and next-12-month document, maintenance and owner-requirement dates in a grouped timeline, jump back to each source workflow and export future compliance events as an all-day iCalendar file.
...
Version 7.1.0 adds Ownership Budgets. Set annual service, touring and upgrade allowances; compare current-year recorded costs with each budget; retain planning notes offline; and surface overspend on the Operations Dashboard.
...
Version 5.7.0 adds Touring Cost Insights. Review all-time, 12-month or 30-day spending, compare visual category shares and trip totals, calculate €/km and L/100 km when the required records exist, and export the selected period as CSV.
